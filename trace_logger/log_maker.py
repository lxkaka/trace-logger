...
class TraceLogger(object):
    ...
    def entry_log(self, **kwargs):
        ...
        INITIAL_PARAMS.tracelogger_user_data = kwargs.pop("user_data", "")
        ...
        if trace_id and rpc_id:
            ...
            INITIAL_PARAMS.tracelogger_trace_id = trace_id
            INITIAL_PARAMS.tracelogger_rpc_id = rpc_id
            ...
            _message = "{trace_id}|{ts}|{rpc_type}|{rpc_id}|{service}|{method}|{code}|{remote_ip}|{span}|{res_size}|" \
                       "{ext_info}|{user_data}".format(
                        trace_id=INITIAL_PARAMS.tracelogger_trace_id,
                        ts=HandleArgs.pretty_string(time_stamp),
                        rpc_type=rpc_type,
                        rpc_id=INITIAL_PARAMS.tracelogger_rpc_id,
                        service=url,
                        ...
                        ext_info=ext_info,
                        user_data=INITIAL_PARAMS.tracelogger_user_data
                        )
    ...
    def client_log(self, **kwargs):
        ...
        user_data = INITIAL_PARAMS.tracelogger_user_data or ""
        time_stamp = kwargs.pop("start_time", "")
        url = kwargs.pop("url", "")
    ...
    @staticmethod
    def transfer_tracelogger_params(header):
        INITIAL_PARAMS.eagleeye_rpc_id = RpcIdUpdater.update_rpc_id(INITIAL_PARAMS.tracelogger_rpc_id)
        tracelogger_header = {
            "EagleEye-TraceId": INITIAL_PARAMS.tracelogger_trace_id,
            "EagleEye-RpcId": INITIAL_PARAMS.tracelogger_rpc_id,
            "EagleEye-UserData": INITIAL_PARAMS.tracelogger_user_data
        }

        return header.update(tracelogger_header)
